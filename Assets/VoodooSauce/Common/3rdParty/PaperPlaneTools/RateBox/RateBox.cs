...
namespace PaperPlaneTools
{
    ...
    public class RateBox
    {
        ...
        public static string GetStoreUrl(string iTunesAppId, string googlePlayMarketAppBundleId)
        {
            var url = "";
#if (UNITY_IPHONE || UNITY_EDITOR)
            url = $"https://itunes.apple.com/app/id{UnityWebRequest.EscapeURL(iTunesAppId)}?action=write-review";
#endif
#if UNITY_ANDROID
			url = $"https://play.google.com/store/apps/details?id={UnityWebRequest.EscapeURL(googlePlayMarketAppBundleId)}";
#endif
            return url;
        }
        ...
        public void Show()
        {
            if (DefaultTextSettings == null) {
                DebugLog(
                    "Can't show a dialog because dialog strings are not configured. Please drag PaperPlaneTools/Resources/RateBox/RateBoxPrefab to your scene or set DefaultTextSettings manually.");
                return;
            }

            Show(DefaultTextSettings.Title, DefaultTextSettings.Message, DefaultTextSettings.RateButtonTitle, DefaultTextSettings.PostponeButtonTitle,
                DefaultTextSettings.RejectButtonTitle);
        }
        ...
        public void ForceShow(string title, string message, string rateButtonTitle, string postponeButtonTilte, string rejectButtonTitle = null)
        {
            Alert alert = new Alert(title, message)
                         .SetPositiveButton(rateButtonTitle, () => { GoToRateUrl(); })
                         .SetNeutralButton(postponeButtonTilte)
                         .AddOptions(new AlertIOSOptions() {
                              PreferableButton = Alert.ButtonType.Positive
                          });

            // This button label must not be empty, otherwise a null reference
            // could happen later in the code. SO a default title is set here.
            // More details here https://voodoo.atlassian.net/browse/VS-4700
            if (string.IsNullOrEmpty(rejectButtonTitle)) {
        ...
        public void ForceShow()
        {
            ...
            ForceShow(DefaultTextSettings.Title, DefaultTextSettings.Message, DefaultTextSettings.RateButtonTitle,
                DefaultTextSettings.PostponeButtonTitle, DefaultTextSettings.RejectButtonTitle);
        }
        ...
        public bool CheckConditionsAreMet()
        {
            ...
            if (Statistics.DialogIsRejected) {
                DebugLog(
                    "Conditions are NOT met because dialog was rejected; DialogIsRejected == true, this flag will be cleared after new application version is detected.");
                return false;
            }

            if (Statistics.DialogIsRated) {
                DebugLog(
                    "Conditions are NOT met because user has already rate the app; DialogIsRated == true, this flag will be cleared after new application version is detected.");
                return false;
            }
