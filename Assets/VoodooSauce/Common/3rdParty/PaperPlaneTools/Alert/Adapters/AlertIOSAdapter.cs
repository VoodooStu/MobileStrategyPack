        ...
        void IAlertPlatformAdapter.Show(Alert alert)
        {
            ...
            foreach (Alert.ButtonType buttonType in buttonsAddOrder) {
                if (buttonType == Alert.ButtonType.Positive && alert.PositiveButton != null) {
                    alertIOS.AddButton(options.PositiveButton, alert.PositiveButton.Title, alert.PositiveButton.Handler,
                        options.PreferableButton == Alert.ButtonType.Positive);
                }

                if (buttonType == Alert.ButtonType.Neutral && alert.NeutralButton != null) {
                    alertIOS.AddButton(options.NeutralButton, alert.NeutralButton.Title, alert.NeutralButton.Handler,
                        options.PreferableButton == Alert.ButtonType.Neutral);
                }

                if (buttonType == Alert.ButtonType.Negative && alert.NegativeButton != null) {
                    alertIOS.AddButton(options.NegativeButton, alert.NegativeButton.Title, alert.NegativeButton.Handler,
                        options.PreferableButton == Alert.ButtonType.Negative);
                }
            }
