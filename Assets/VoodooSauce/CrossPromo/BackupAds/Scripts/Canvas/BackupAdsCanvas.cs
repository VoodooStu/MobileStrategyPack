...
namespace Voodoo.Sauce.Internal.CrossPromo.BackupAds.Scripts.Canvas
{
    public class BackupAdsCanvas : MonoBehaviour
    {
        ...
        {
            if (!_delayElapsed) {
                float elapsed = _delayBeforeShowingCloseButton - (Time.unscaledTime - _startTime);
                if (elapsed > 0) {
        ...
        private void AdjustDuration(VideoPlayer player)
        {
            ...
            if (_delayBeforeShowingCloseButton <= -1f ||
                videoDuration > 0 && _delayBeforeShowingCloseButton > videoDuration) {
                _delayBeforeShowingCloseButton = (float)videoDuration;
            }
            VoodooLog.LogDebug(Module.ADS, TAG, $"BackupAds closing time: {_delayBeforeShowingCloseButton}");
        ...
        {
            if (_info.videoClip) {
                VoodooLog.LogDebug(Module.CROSS_PROMO, TAG, $"Starting video from clip: {_info.videoClip}");
                videoPlayer.PlayVideo(_info.videoClip);
            } else if (!string.IsNullOrEmpty(_info.videoUrl)) {
                VoodooLog.LogDebug(Module.CROSS_PROMO, TAG, $"Starting video from url: {_info.videoUrl}");
                videoPlayer.PlayVideo(_info.videoUrl);
            } else {
                VoodooLog.LogDebug(Module.CROSS_PROMO, TAG, $"No clip or URL found.");
            }
        }
