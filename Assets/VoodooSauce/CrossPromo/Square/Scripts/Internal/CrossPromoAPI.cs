...
{
    internal class CrossPromoAPI : MonoBehaviour
    {
        ...
        private const string CP_SQUARE_STRATEGY_ID = "cp_square_strategy_id";

        internal static CrossPromoAPI CreateGameObject()
        {
            var gameObject = new GameObject("CrossPromoAPI");
            var restClient = gameObject.AddComponent<CrossPromoAPI>();
            return restClient;
        }
        ...
        /// <summary>
        /// Get the information about the game and download the asset in the cache.
        /// </summary>
        ...
        /// <summary>
        /// Check if the Cross Promo is downloading.
        /// </summary>
        /// <returns>True if is in currently in job</returns>
        public bool IsRetrievingInfo() => _isRetrievingInfo;

        /// <summary>
        /// Coroutine
        /// Retrieve the information about the current game
        /// Download files in cache
        /// </summary>
        ...
        private IEnumerator StartGetGameInfo()
        {
            ...
            if (webRequest.isNetworkError || webRequest.isHttpError) {
                ...
                VoodooLog.LogWarning(Module.CROSS_PROMO, VoodooCrossPromo.TAG,
            ...
            try {
                if (!string.IsNullOrEmpty(webRequest.downloadHandler.text))
                    ...
                    VoodooLog.LogError(Module.CROSS_PROMO, VoodooCrossPromo.TAG, "JSON Empty");
            } catch (Exception e) {
        ...
        /// <summary>
        /// Download files inside the cache
        /// </summary>
        private static IEnumerator DownloadFiles()
        {
            ...
            foreach (AssetModel asset in assets) {
                ...
                if (wwwFile.error == null) {
                    ...
                    VoodooLog.LogWarning(Module.CROSS_PROMO, VoodooCrossPromo.TAG, "Failed to download file in cache: " + wwwFile.error);
            }
        ...
        /// <summary>
        /// Clean the cache. Delete the files if no exist in the API
        /// </summary>
        ...
        private static IEnumerator StartWaitForFirstFrame(Action action)
        {
            ...
            if (AFormatManager.Instance == null)
                VoodooLog.LogWarning(Module.CROSS_PROMO, VoodooCrossPromo.TAG, "No prefab found.");
            else
                action();
        }
