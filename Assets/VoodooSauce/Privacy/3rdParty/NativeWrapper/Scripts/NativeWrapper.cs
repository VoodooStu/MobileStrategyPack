...
namespace Voodoo.Sauce.Privacy
{
    internal class NativeWrapper
    {
        ...
        internal static void RequestAuthorization(RequestAuthorizationCallback callback)
        {
            ...
            {
                VoodooLog.LogError(Module.PRIVACY, TAG, "It's impossible to display the ATT popup in the Unity editor.");
                return;
            }
        ...
        internal static Task<IdfaAuthorizationStatus> RequestAuthorization()
        {
            var tcs = new TaskCompletionSource<IdfaAuthorizationStatus>();
            RequestAuthorization(status => tcs.TrySetResult(status));
            return tcs.Task;
        }
